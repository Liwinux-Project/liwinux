...
S "3. Native crash (tombstone) - does it belong to system_server"
...
L logcat -b main -d | grep -icE "Entered the Android system server" | sed 's/^/  start count: /'
...
S "6. HOST memory state (the real suspect)"
free -h | sed 's/^/  /'
...
journalctl -k --since "1 hour ago" --no-pager 2>/dev/null | grep -iE "out of memory|oom-kill|killed process" | tail -10 || echo "    (no kernel OOM record)"

S "7. How heavy GAPPS is (the difference from VANILLA)"
