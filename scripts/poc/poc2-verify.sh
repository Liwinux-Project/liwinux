#!/usr/bin/env bash
# liwinux PoC 2 verification - GPU, ARM, microG and network
set -u
...
echo "  total packages: $(W sh -c 'pm list packages 2>/dev/null | wc -l' | tail -1)"

S "5. NETWORK ACCESS (required by Aurora and Play)"
...
W sh -c 'sqlite3 /data/data/com.google.android.gsf/databases/gservices.db "select * from main where name = \"android_id\"" 2>/dev/null || echo "(no sqlite3, or GSF has not run yet)"'
