#!/usr/bin/env bash
# liwinux PoC 1 — duzeltilmis dogrulama ("--" ayraci ile)
set -u
W() { waydroid --details-to-stdout shell -- "$@" 2>&1 | tr -d '\r'; }
S() { echo; echo "=== $* ==="; }

S "1. libhoudini guest icinde gorunuyor mu"
...
W sh -c '/system/bin/houdini64 --version 2>&1 | head -5; echo "cikis=$?"'
...
S "6. logcat: GPU tarafi hata var mi"
waydroid --details-to-stdout shell -- logcat -d 2>&1 | tr -d '\r' \
